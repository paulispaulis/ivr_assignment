...
class Get_Coords:
  ...
  def jacobian(self):
    ...
    j = np.array([[2*(-s(1))*c(2)*c(3)*c(4) + 2*c(1)*s(3)*s(4) + 2*(-s(1))*s(2)*s(4) + 3*(-s(1))*c(2)*c(3) + 3*c(1)*s(3),
      ...
      2*c(1)*c(2)*(-s(3))*c(4) + 2*s(1)*c(3)*c(4) + 3*c(1)*c(2)*(-s(3)) + 3*s(1)*c(3),
      2*c(1)*c(2)*c(3)*(-s(4)) + 2*s(1)*s(3)*(-s(4)) + 2*c(1)*s(2)*c(4)],
      [2*c(1)*c(2)*c(3)*c(4) + 2*s(1)*s(3)*s(4) + 2*c(1)*s(2)*s(4) + 3*c(1)*c(2)*c(3) + 3*s(1)*s(3),
      2*s(1)*(-s(2))*c(3)*c(4) + 2*s(1)*c(2)*s(4) + 3*s(1)*(-s(2))*c(3),
      2*s(1)*c(2)*(-s(3))*c(4) - 2*c(1)*c(3)*c(4) + 3*s(1)*c(2)*(-s(3)) - 3*c(1)*c(3),
      2*s(1)*c(2)*c(3)*(-s(4)) - 2*c(1)*s(3)*(-s(4)) + 2*s(1)*s(2)*c(4)],
      [0, 
      2*c(2)*c(3)*c(4) + 2*s(2)*s(4) + 3*c(2)*c(3),
